...
Githubの使い方
	...
	競合が発生したときのマージについて
		...
		編集する前に同期しても，編集前の最新版と編集後同期前の最新版が同一とは限りません

		その場合，編集の「競合」が発生したことになります

		競合が発生すると，どのコード（ほかの人のコードと自分のコード）を最新版とするかが問題となります

		そのため，どちらかを最新とするのではなく，多くの場合は
		
		それぞれの編集した内容をマージ（合体）させたものを最新とすることになります


		同期しようとした場合に競合が生じているとき，VisualStudioが自動的にマージを提案してくれます

		「マージする」を押すとマージした場合の結果を提案されます

		これはマージ結果の確定版ではないので，必要に応じてから「マージを許可」します

		マージを許可した後にコミットして同期すると，マージした結果でコミットできます
		

		複数人でコードの違う部分を編集する場合，競合を防ぐためにフォーク（分岐）します
